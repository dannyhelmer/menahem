...
function ProviderKeyRow({ providerId, label }: { providerId: string; label: string }) {
  ...
  useEffect(() => {
    fetch(`/api/settings/ai-provider-keys/${providerId}`)
      .then((res) => res.json())
      .then(setStatus);
  }, [providerId]);
  ...
  async function handleSave() {
    ...
    setSaving(true);
    const res = await fetch(`/api/settings/ai-provider-keys/${providerId}`, {
      method: "PUT",
  ...
  async function handleClear() {
    setSaving(true);
    const res = await fetch(`/api/settings/ai-provider-keys/${providerId}`, { method: "DELETE" });
    setStatus(await res.json());
    setSaving(false);
  }
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="w-32 shrink-0 text-sm text-neutral-700 dark:text-neutral-300">{label}</div>
      {status?.configured ? (
        <>
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-neutral-500 dark:text-neutral-400">
        ...
        encrypted at rest and never shared with other accounts. If both are configured, OpenAI is used first.
      </p>
      <ProviderKeyRow providerId="openai" label="OpenAI" />
      <ProviderKeyRow providerId="anthropic" label="Claude (Anthropic)" />
    </div>
  );
}
